model = dict(
    ...
    backbone=dict(  # 主干网络的配置文件
        ...
        dcn=dict(type='DCN', deformable_groups=1, fallback_on_stride=False,stage_with_dcn=(False, True, True, True))
        ),  # 加载通过 ImageNet 预训练的模型
    neck=dict(
        type='PAFPN',  # 检测器的 neck 是 FPN，我们同样支持 'NASFPN', 'PAFPN' 等，更多细节可以参考 https://mmdetection.readthedocs.io/en/latest/api.html#mmdet.models.necks.FPN
    ...
    rpn_head=dict(
        ...
        loss_bbox=dict(  # 回归分支的损失函数配置
            type='SmoothL1Loss',  # 损失类型，我们还支持许多 IoU Losses 和 Smooth L1-loss 等，更多细节请参考 https://github.com/open-mmlab/mmdetection/blob/main/mmdet/models/losses/smooth_l1_loss.py#L56
    ...
    roi_head=dict(  # RoIHead 封装了两步(two-stage)/级联(cascade)检测器的第二步
        ...
        bbox_head=dict(  # RoIHead 中 box head 的配置
            ...
            loss_bbox=dict(  # 回归分支的损失函数配置
                type='SmoothL1Loss',  # 损失类型，我们还支持许多 IoU Losses 和 Smooth L1-loss 等
                loss_weight=1.0)),  # 回归分支的损失权重
